...
export function registerTasteCommand(program: Command): void {
  ...
  taste
    ...
    .option(
      "--merge",
      "Merge with existing taste profile instead of replacing",
      true,
    )
    .action(
      async (
        ...
        logger.info(
          `Analyzing: ${analyses.map((a) => chalk.cyan(a)).join(", ")}`,
        );
        logger.raw("");
        ...
        try {
          ...
          if (learned.formatting) {
            ...
            logger.raw(
              `     Semicolons:       ${f.semi ? chalk.green("yes") : chalk.red("no")}`,
            );
            logger.raw(
              `     Quotes:           ${f.singleQuote ? chalk.green("single") : chalk.blue("double")}`,
            );
            logger.raw(
              `     Trailing commas:  ${f.trailingComma ? chalk.green("yes") : chalk.red("no")}`,
            );
            logger.raw(
              `     Tab width:        ${chalk.yellow(String(f.tabWidth))}`,
            );
            logger.raw("");
          }
          ...
          if (learned.styling && learned.styling.approach !== "unknown") {
            logger.raw(
              `  ${chalk.bold("🎨 Styling:")}       ${chalk.cyan(learned.styling.approach)}`,
            );
          }
          ...
          if (learned.testing && learned.testing.framework !== "unknown") {
            logger.raw(
              `  ${chalk.bold("🧪 Testing:")}       ${chalk.cyan(learned.testing.framework)}`,
            );
            logger.raw(
              `  ${chalk.bold("   Pattern:")}       ${chalk.cyan(learned.testing.pattern)}`,
            );
            logger.raw("");
          }
          ...
          if (learned.learnedPatterns && learned.learnedPatterns.length > 0) {
            ...
            for (const p of learned.learnedPatterns) {
              const bar = chalk.green(
                "█".repeat(Math.round(p.confidence * 10)),
              );
              const empty = chalk.gray(
                "░".repeat(10 - Math.round(p.confidence * 10)),
              );
          ...
          logger.raw("");
          logger.info(
            `Run ${chalk.cyan("ctx taste profile")} to see your full taste profile.`,
          );
          logger.info(
            `Run ${chalk.cyan("ctx init")} to generate context files with your taste baked in.`,
          );
        } catch (err) {
  ...
  taste
    ...
    .action((opts: { json: boolean }) => {
      ...
      if (
        profile.sourceProjects.length === 0 ||
        profile.confidence.overall < 0.1
      ) {
        logger.raw("");
        logger.info(
          `No taste learned yet. Run ${chalk.cyan("ctx taste learn <project-path>")} to start.`,
        );
      }
  ...
  taste
    .command("code")
    .description(
      "Deep code analysis — learn taste from your actual source code patterns",
    )
    .argument("[path]", "Path to the project to analyze", ".")
    .option("--json", "Output as JSON", false)
    .option("--no-merge", "Do not merge analysis into the taste profile")
    .action(
      async (
        projectPath: string,
        opts: {
        ...
        const spinner = ora({
          text: "Reading source files and analyzing patterns...",
          isSilent: opts.json,
        }).start();
        ...
        try {
          ...
          spinner.succeed(
            `Analyzed ${chalk.bold(String(analysis.totalFilesAnalyzed))} source files`,
          );
          logger.raw("");
          ...
          const medCount = suggestions.filter(
            (s) => s.severity === "medium",
          ).length;
          ...
          logger.info(
            `Patterns: ${chalk.green(String(analysis.totalFilesAnalyzed))} files analyzed`,
          );
          if (suggestions.length > 0) {
            logger.info(
              `Suggestions: ${chalk.red(highCount.toString())} high, ${chalk.yellow(medCount.toString())} medium, ${chalk.green(lowCount.toString())} low priority`,
            );
          }
          logger.raw("");
          ...
          if (opts.merge) {
            ...
            learned.imports = {
              style:
                analysis.imports.namedVsDefault > 0.6
                  ? "named"
                  : analysis.imports.namedVsDefault < 0.3
                    ? "default"
                    : "mixed",
            ...
            // Component style
            learned.components = {
              pattern: analysis.components.usesFunctionalComponents
                ? "functional"
                : "class",
            ...
            learned.naming = {
              ...
              functions:
                analysis.functions.arrowVsDeclaration > 0.7
                  ? "camelCase"
                  : "camelCase",
              types:
                analysis.typescript.interfaceVsType > 0.6
                  ? "PascalCase"
                  : "PascalCase",
            ...
            learned.testing = {
              ...
              pattern: analysis.organization.coLocatedTests
                ? "co-located"
                : "separate-dir",
            };
            ...
            if (analysis.functions.asyncPercentage > 0.3) {
              learnedPatterns.push({
                ...
                evidence: [
                  `${analysis.totalFilesAnalyzed} source files analyzed`,
                ],
              });
            }
            ...
            if (analysis.typescript.usesGenerics) {
              learnedPatterns.push({
                ...
                description:
                  "Actively uses TypeScript generics for type safety",
                confidence: 0.6,
                evidence: [`Detected in codebase`],
              });
            }
            if (analysis.typescript.anyUsage < 0.3) {
              learnedPatterns.push({
                ...
                confidence: 0.8,
                evidence: [
                  `Low any usage detected (${Math.round(analysis.typescript.anyUsage * 100)}% of files)`,
                ],
              });
            }
            ...
            if (analysis.errorHandling.hasCustomErrors) {
              learnedPatterns.push({
                ...
                description:
                  "Defines custom error classes for better error handling",
                confidence: 0.7,
                evidence: [`Detected in codebase`],
              });
            }
            ...
            logger.raw("");
            logger.info(
              `New confidence: ${chalk.bold(Math.round(merged.confidence.overall * 100) + "%")}`,
            );
            logger.info(
              `Run ${chalk.cyan("ctx taste profile")} to see the updated profile.`,
            );
          }
