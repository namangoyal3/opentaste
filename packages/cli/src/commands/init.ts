  ...
  program
    ...
    .action(async (projectPath: string, opts: { 
      ...
      if (!opts.json) {
        ...
        logger.raw(`  Languages:  ${project.languages.map(l => chalk.cyan(l.name)).join(', ')}`);
        logger.raw(`  Frameworks: ${project.frameworks.length > 0 ? project.frameworks.map(f => chalk.green(f.name)).join(', ') : chalk.dim('none detected')}`);
        if (project.packageManager) logger.raw(`  Package:    ${chalk.yellow(project.packageManager)}`);
        if (project.buildTool) logger.raw(`  Build:      ${chalk.magenta(project.buildTool)}`);
        if (project.testFramework) logger.raw(`  Tests:      ${chalk.blue(project.testFramework)}`);
      ...
      if (opts.template) {
        ...
        if (template) {
          ...
          logger.warn(`Template "${opts.template}" not found. Using auto-generated context.`);
        }
      }

      // Write files
      const filesToWrite: Array<{ path: string; content: string; label: string }> = [];
      ...
      if (opts.dryRun) {
        ...
        logger.raw(`Would write ${chalk.bold(filesToWrite.length.toString())} file(s):`);
        for (const file of filesToWrite) {
          logger.raw(`  ${chalk.cyan(file.label)} → ${file.path}`);
        }
        if (opts.json) {
          logger.raw(JSON.stringify({ files: filesToWrite.map(f => ({ path: f.path, label: f.label })) }, null, 2));
        }
        return;
      }
      ...
      for (const file of filesToWrite) {
        try {
          ...
          logger.error(`Failed to write ${file.label}: ${err instanceof Error ? err.message : 'Unknown error'}`);
        }
      }
      ...
      logger.raw('');
      logger.success(`Context generated! Your AI tools now have optimal project understanding.`);
    });
}
